...
from code_loader.inner_leap_binder.leapbinder_decorators import tensorleap_preprocess, tensorleap_input_encoder, \
    tensorleap_gt_encoder, tensorleap_custom_visualizer, tensorleap_custom_loss, tensorleap_metadata
...
def custom_loss(predictions, targets):
    ...
    def compute_yolo_cross_entropy_loss(predictions, targets, iou_threshold=0.5,
                                        num_classes=80, negative_weight=0.1):
        """
        ...
        matched_pairs, unmatched_preds, unmatched_targets = match_predictions_to_targets(
            pred_np, target_np, iou_threshold
        )
        ...
        # Penalty for unmatched targets (missed detections)
        missed_detection_penalty = len(unmatched_targets) * 2.0  # Penalty for each missed target
